...
const About = () => {
  return (
    <section id="hakkimizda" className="py-20 lg:py-32 bg-white overflow-hidden relative">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-24">
          ...
          <div className="relative w-full lg:w-1/2 min-h-[300px] lg:min-h-[500px] flex items-center justify-center">
            {/* Dekoratif Arka Plan - Mobilde biraz küçültüldü */}
            <div className="absolute -bottom-5 -left-5 w-40 h-40 lg:w-80 lg:h-80 bg-[#8B1A1A]/5 rounded-[2rem] lg:rounded-[4rem] -z-0"></div>
            
            {/* Görsel Çerçevesi - Mobilde h-600'den h-[350px]'e çekildi */}
            ...
            {/* Tecrübe Rozeti - Mobilde daha kompakt yapıldı */}
            <div className="absolute bottom-6 -right-2 lg:bottom-12 lg:-right-6 z-20 bg-[#8B1A1A] text-white p-4 lg:p-8 rounded-[1.5rem] lg:rounded-[2.5rem] shadow-2xl transform rotate-3 hover:rotate-0 transition-all duration-500">
              ...
              <div className="text-[8px] lg:text-[11px] font-bold uppercase tracking-[0.15em] lg:tracking-[0.25em] leading-tight opacity-90">Yıllık Eğitim<br/>Tecrübesi</div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 pt-4 lg:pt-0">
            ...
            <h3 className="text-4xl md:text-8xl font-[1000] text-black tracking-tighter leading-[1.05] mb-8 lg:mb-12 uppercase">
              TORBALI'NIN <br />EĞİTİMDEKİ <br />
              <span className="text-[#8B1A1A] relative">
                ALFA'SI
                <span className="absolute -bottom-1 left-0 w-full h-2 lg:h-4 bg-[#8B1A1A]/10 -z-10"></span>
              </span>
            </h3>
            
            <p className="text-lg lg:text-2xl text-gray-600 font-bold leading-relaxed lg:leading-[1.8] mb-10 lg:mb-14">
            ...
            {/* Özellikler: MOBİLDE 2X2 GRID YAPILDI */}
            <div className="grid grid-cols-2 lg:grid-cols-2 gap-y-8 gap-x-4 lg:gap-x-16">
              {[
                ...
                <div key={index} className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-3 lg:gap-6 group">
                  <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full border-2 border-[#8B1A1A] flex items-center justify-center group-hover:bg-[#8B1A1A] transition-all shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 font-bold text-[8px] lg:text-xs uppercase tracking-wider">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
