...
const Footer = () => {
  ...
  // Logo URL belirleme (Admin panelinden gelen logo yoksa varsayılanı kullan)
      ...
      <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="col-span-1 text-right lg:text-left">
            ...
            <ul className="space-y-2">
              <li className="flex items-center justify-end lg:justify-start space-x-1.5">
                <span className="text-gray-400 text-[8px] md:text-xs font-medium leading-none">
                   {settings?.contactInfo?.address || "Torbalı / İzmir"}
                </span>
                <MapPin className="text-[#8B1A1A] w-2.5 h-2.5 shrink-0" />
              </li>
              <li className="flex items-center justify-end lg:justify-start space-x-1.5">
                <a href={`tel:${settings?.contactInfo?.phone}`} className="text-gray-400 text-[8px] md:text-xs font-black tracking-tighter">
        ...
        <div className="flex flex-col items-center py-4 border-t border-white/5 space-y-2.5">
          ...
          <div className="flex space-x-3">
            <a href={settings?.socialMedia?.instagram || "#"} target="_blank" rel="noopener noreferrer" className="w-6 h-6 bg-white/5 rounded-md flex items-center justify-center hover:bg-[#8B1A1A] transition-all">
              <Instagram className="w-3.5 h-3.5" />
            </a>
            <a href={settings?.socialMedia?.facebook || "#"} target="_blank" rel="noopener noreferrer" className="w-6 h-6 bg-white/5 rounded-md flex items-center justify-center hover:bg-[#8B1A1A] transition-all">
