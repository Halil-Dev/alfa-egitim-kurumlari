...
export default async function DuyuruDetay({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Kurum WhatsApp Numarası (Burayı güncellemeyi unutma)
  ...
  return (
    <main className="py-20 bg-white min-h-screen">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <h1 className="text-4xl md:text-6xl font-[1000] text-black mb-8 uppercase tracking-tighter leading-none">
          {post.title}
        </h1>
        
        {post.image && (
          <div className="relative h-[400px] md:h-[500px] w-full mb-10 rounded-[2.5rem] overflow-hidden shadow-2xl border border-gray-100">
        ...
        <div className="text-center bg-[#f0fff4] p-10 rounded-[3rem] border-2 border-dashed border-[#25D366]">
          <h3 className="text-2xl font-black mb-4">Aklınıza takılan bir şey mi var?</h3>
          <p className="text-gray-600 mb-8 font-bold text-lg">Hemen eğitim danışmanımızla iletişime geçin.</p>
          <a 
            href={`https://wa.me/${WHATSAPP_NUMBER}?text=${message}`}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-4 bg-[#25D366] text-white px-12 py-6 rounded-full font-black uppercase tracking-tighter hover:scale-105 transition-transform shadow-2xl"
          >
